            ...
            <ul>
                <li v-for="item in folderList" :key="item.contractId" :id='item.contractFolderId'>
                    <div class="contract-folder" :id='item.folderId'>
                        ...
                        <span :id='item.folderId' @click='open(item)' :class="{'colorActive': item.contractActive}" @contextmenu.prevent="handle($event,item)" class="no-chase cursor-pointer">{{item.folderName}}</span>
                        ...
                        <ul v-if="item.folderActive" style="padding-left: 15px;" v-Clickoutside="checkNull">
                            <li class="contract-file" v-for='list in item.child' :key="list.id">
                                ...
                                <span @click='select(list)' @contextmenu.prevent="handleFile($event,list)" :id='list.id' :class="{'colorActive': list.contractActive}">{{list.contractName}} </span>

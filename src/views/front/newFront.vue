      ...
      <div class="search-part" style="padding-top: 20px">
        <div class="search-part-left" v-if="!disabled">
          <el-button
            ...
            @click="createFront"
            >{{ this.$t("nodes.addFront") }}</el-button
          >
        </div>
      </div>
          ...
          >
            <template slot-scope="scope">
              <template v-if="head.enName != 'operate'">
                ...
                <span v-else-if="head.enName === 'nodeType'">{{
                  nodeText(scope.row[head.enName])
                }}</span>
...
export default {
  ...
  methods: {
    ...
    showDetail(item) {
      ...
      this.$router.push({
        path: "hostDetail",
        query: { nodeIp: item.nodeIp, nodeId: item.nodeId },
      });
    },
    deletedFront(val) {
      this.$confirm(this.$t("text.confirmDelete"))
        .then((_) => {
          deleteFront(val.frontId)
            .then((res) => {
              if (res.data.code === 0) {

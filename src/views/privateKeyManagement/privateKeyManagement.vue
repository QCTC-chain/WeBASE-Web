  ...
  <div>
      ...
      <div class="search-part">
        <div class="search-part-left" v-if="!disabled">
          <el-button
            ...
            v-hasPermi="['bcos:privateKeyManagement:userOperate']"
            >{{ this.$t("privateKey.addUser") }}</el-button
          >
          <el-button
            type="primary"
            class="search-part-left-btn"
            ...
            v-hasPermi="['bcos:privateKeyManagement:userOperate']"
            >{{ this.$t("privateKey.importPrivateKey") }}</el-button
          >
          <el-tooltip
            effect="dark"
            :content="$t('privateKey.addUserTips')"
            placement="top-start"
          >
        ...
        <div class="search-part-right">
          <el-input
            :placeholder="$t('privateKey.searchUser')"
            v-model="userName"
          ...
          >
            <template slot-scope="scope">
              <template v-if="head.enName != 'operate'">
                <span
                  v-if="head.enName === 'userStatus'"
                  :style="{ color: statusColor(scope.row[head.enName]) }"
                  >{{ userStatus(scope.row[head.enName]) }}</span
                >
                ...
                <span v-else-if="head.enName === 'userId'">
                  ...
                  >
                    <i
                      class="wbs-icon-key-b font-12"
                      :style="{
                        color: scope.row['hasPk'] == 1 ? '#FFC31F' : '#4F9DFF',
                      }"
                    ></i>
                  </el-tooltip>
                  {{ scope.row[head.enName] }}
              ...
              <template v-else>
                ...
                <el-button
                  ...
                  size="small"
                  :class="{ grayColor: disabled }"
                  @click="bindPrivateKey(scope.row)"
                  v-hasPermi="['bcos:privateKeyManagement:userOperate']"
                  >{{ $t("text.addContractAddress") }}</el-button
                >
                <el-button
                  :disabled="disabled"
                  type="text"
    ...
    >
      <v-creatUser
        ...
        @bindUserClose="bindUserClose"
        ref="creatUser"
      ></v-creatUser>
    </el-dialog>
    <el-dialog
      :visible.sync="$store.state.importPrivateKey"
      :title="$t('privateKey.importPrivateKey')"
      width="640px"
      :append-to-body="true"
      class="dialog-wrapper"
    ...
    <el-dialog
      :visible.sync="$store.state.exportRivateKey"
      :title="$t('system.export')"
      width="640px"
      :append-to-body="true"
      class="dialog-wrapper"
      v-if="$store.state.exportRivateKey"
      center
    >
      <export-key :exportInfo="exportInfo"></export-key>
    </el-dialog>
    <el-dialog
      :visible.sync="$store.state.bindKey"
      :title="$t('privateKey.bindPrivateKey')"
      width="640px"
      :append-to-body="true"
      class="dialog-wrapper"
      v-if="$store.state.bindKey"
      center
    >
      <bind-key
        :bindInfo="bindInfo"

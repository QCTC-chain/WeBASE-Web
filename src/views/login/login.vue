...
<template>
  <div class="login-bg" :style="{ backgroundImage: 'url(' + bgLogin + ')' }">
    ...
    <div class="login">
      ...
      <div class="loginTip">
  <el-popover class="forgetPassWord"
    placement="top"
    title=""
    width="250"
    trigger="click"
    ...
    <a target="_blank" style="color:cornflowerblue" href="https://webasedoc.readthedocs.io/zh_CN/latest/docs/WeBASE-Node-Manager/appendix.html#forget_pwd">{{$t('login.ForgetPassword')}}</a>
    <span>{{$t('login.FollowPrompt')}}</span>
    <el-button slot="reference">{{$t('login.ForgetPassword')}}</el-button>
  </el-popover>
  <el-popover class="pictureCheckCodeErr" v-show="pictureCheckCodeErrShow"
    placement="top"
    title=""
    width="250"
    trigger="click"
    ...
    <a target="_blank" style="color:cornflowerblue" href="https://webasedoc.readthedocs.io/zh_CN/latest/docs/WeBASE-Web/appendix.html#code_fail">{{$t('login.VerificaFailed')}}</a>
    <span>{{$t('login.FollowPrompt')}}</span>
      ...
      getPictureCheckCode()
        .then((res) => {
          if (res.data.code === 0) {
            ...
            this.codeUrl = "";
            this.authToken = "";
            this.pictureCheckCodeErrShow=true;
            this.$message({

<template>
  <div>
    ...
    <nav-menu
      ...
      :headSubTitle="$t('title.certificate')"
    ></nav-menu>
      ...
      <div class="search-part" style="display: flex; padding: 20px 40px">
        ...
        <div style="margin-left: 20px">
          <el-button
            ...
            @click="exportSdk()"
            size="small"
            v-hasPermi="['bcos:sys:downloadSdkCert']"
            >{{ $t("text.exportSdk") }}</el-button
          >
        </div>
      </div>
            ...
            <template slot-scope="scope">
                ...
                <template v-if="scope.row[head.enName]">
                  ...
                  <span v-else-if="head.enName === 'fingerPrint'">
                    <i
                      ...
                      @click="copyFingerPrint(scope.row[head.enName])"
                      :title="$t('text.copy')"
                    ></i>
                  ...
                  <span v-else-if="head.enName === 'validityFrom'">
                    {{
                      format(scope.row["validityFrom"], "yyyy-MM-dd HH:mm:ss")
                    }}
                    {{ $t("system.to") }}
                    {{ format(scope.row["validityTo"], "yyyy-MM-dd HH:mm:ss") }}
                  </span>
                  <span v-else>
                    {{ scope.row[head.enName] }}
                  </span>
              ...
              <template v-else>
                <el-button
                  ...
                  v-hasPermi="['bcos:sys:deleteCert']"
    ...
    <el-dialog
      :visible.sync="frontDialogVisible"
      :title="frontDialogTitle"
      width="433px"
      :append-to-body="true"
      :center="true"
      class="dialog-wrapper"
      v-if="frontDialogVisible"
    >
  ...
  methods: {
    ...
    onUploadExceed(files, fileList) {
      this.$message.warning(
        this.$t("system.uploadWarning1") +
          files.length +
          this.$t("system.uploadWarning2")(files.length + fileList.length) +
          this.$t("system.uploadWarning3")
      );
    },
    close() {

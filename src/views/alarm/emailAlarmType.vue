<template>
  <div>
    ...
    <nav-menu
      :headTitle="$t('alarm.alarmCofig')"
      :headSubTitle="$t('alarm.alarmTypeConfig')"
    ...
    <div class="module-wrapper" style="padding: 20px">
      <div style="padding-bottom: 10px">
        ...
        <el-switch
          ...
          v-hasPermi="['bcos:monitor:toggleAlertRules']"
        >
        </el-switch>
      </div>
      ...
      >
        ...
        <el-table-column
          :label="$t('alarm.emailContent')"
          prop="alertContent"
          show-overflow-tooltip
          align="center"
        ></el-table-column>
        <el-table-column
          :label="$t('alarm.sendTime')"
          prop="alertIntervalSeconds"
          show-overflow-tooltip
          align="center"
        >
          <template slot-scope="scope">
            <span v-if="scope.row.alertIntervalSeconds > 1799"
              >{{ scope.row.alertIntervalSeconds / 3600
              }}{{ $t("alarm.hours") }}</span
            >
            <span v-else
              >{{ scope.row.alertIntervalSeconds / 60
              }}{{ $t("alarm.minute") }}</span
            >
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('alarm.operation')"
          fixed="right"
          show-overflow-tooltip
          align="center"
        >
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="small"
              v-hasPermi="['bcos:monitor:updateAlertRule']"
              >{{ $t("alarm.update") }}</el-button
            >
            <el-button
    ...
    <div class="module-wrapper" style="padding: 20px; margin-top: 10px">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span v-if="scope.row.alertType == 1">{{
              $t("alarm.nodeAlarm")
            }}</span>
            <span v-if="scope.row.alertType == 2">{{
              $t("alarm.auditAlert")
            }}</span>
        ...
        >
          <template slot-scope="scope">
            <span v-if="scope.row.alertLevel == 1" style="color: #f00">{{
              $t("alarm.high")
            }}</span>
            <span v-if="scope.row.alertLevel == 2" style="color: #ffd700">{{
              $t("alarm.common")
            }}</span>
        ...
        >
          <template slot-scope="scope">
            <span v-if="scope.row.status" style="color: #3cb371">{{
              $t("alarm.processed")
            }}</span>
    ...
    <emailAlarmType-detail
      :show="emailAlarmTypeShow"
      v-if="emailAlarmTypeShow"
      :data="emailAlarmTypeData"
      @close="emailAlarmTypeClose"
    ></emailAlarmType-detail>
    <emailAlarm-detail
      :show="emailAlarmShow"
      v-if="emailAlarmShow"
      :data="emailAlarmData"
      @close="emailAlarmClose"
    ></emailAlarm-detail>
  </div>
</template>

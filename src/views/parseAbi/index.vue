        ...
        <div class="module-wrapper">
            <div class="ace-wrapper">
                ...
                <div class="parse-button">
                    <el-button type="primary" @click="parseAbi">{{this.$t('text.parse')}}</el-button>
                </div>
            </div>
            <div class="ace-wrapper">
                <h2 class="h2-inscription">{{this.$t('text.parseAbiManually')}}</h2>
                <el-row :gutter="10" style="margin-bottom:10px">
                    <el-col :span="7">
                        <div>{{this.$t('text.functionType')}}</div>
                        <el-select v-model="functionType" @change="changeFunType">
                            <el-option v-for="(item, index) in functionList" :key="index" :label="item" :value="item"></el-option>
                        </el-select>
                    </el-col>
                    <el-col :span="17" v-if="functionType!='constructor'">
                        <span class="font-color-fff text-hidden">name</span>
                        <el-input v-model="functionValue" @input="inputFunctionValue"></el-input>
                    </el-col>
                </el-row>
                <el-row :gutter="10" v-for="(item,index) in argumentList " :key="index" style="margin-bottom:10px">
                    <el-col :span="7">
                        <div>{{item.name}}</div>
            ...
            <div class="add-abi">
                <el-button type="primary" @click="addAbiEncodeStroke">{{this.$t('text.addParameter')}}</el-button>
                <el-button @click="removeAbiEncodeStroke">{{this.$t('text.remove')}}</el-button>
            </div>
            <div class="container copy-text">
                <el-input v-model="textarea" type="textarea" :autosize="{ minRows: 4}"></el-input>
                <el-button type="primary" style="margin: 20px auto;visibility: hidden;" @click="copy">{{this.$t('text.copy')}}</el-button>
            </div>
        </div>
    </div>
